...
- 收到彩信时会转发彩信文字内容，并追加图片、视频或其他附件数量摘要；当前版本不会把彩信附件原图上传到飞书。
- 支持双卡策略：
  - 使用接收短信的 SIM 卡转发。
  - 使用系统默认短信 SIM 卡转发。
  - 指定某张 SIM 卡转发。
    ...
    "post": {
      "zh_cn": {
        ...
        "content": [
          ...
          [
            { "tag": "text", "text": "时间：" },
            { "tag": "text", "text": "2026-05-08 12:34:56" }
          ],
          [
            { "tag": "text", "text": "内容：" }
          ],
...
彩信图片说明：飞书自定义机器人发送图片消息需要 `image_key`，而 `image_key` 需要先调用飞书图片上传接口获取，并携带飞书应用访问凭证。当前自动转发规则只配置 webhook 地址，所以 v1 只发送彩信正文和附件摘要，不直接上传或发送图片原文件。
